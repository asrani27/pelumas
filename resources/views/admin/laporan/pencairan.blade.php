...
<body>
    <table border=0 width="100%">
        ...
        <tr>
            <td colspan=3 style="text-align:center"><br/><strong><u>LAPORAN PENCAIRAN</u></strong></td>
        </tr>
    </table>
    <br/>
    <table border=1 cellspacing="0" cellpadding="3" width="100%">
        ...
        @foreach ($data as $key => $item)
            <tr>
                ...
                <td style="text-align: center">{{number_format($item->jumlah_deposito)}}</td>
                <td style="text-align: center">{{$item->bunga}}</td>
                <td style="text-align: center">{{$item->jangka_waktu}}</td>
                <td style="text-align: center">{{number_format($item->profit_bunga)}}</td>
                <td style="text-align: center">{{number_format($item->pajak)}}</td>
                <td style="text-align: center">{{number_format($item->total_pendapatan)}}</td>
                <td style="text-align: center">{{number_format($item->pencairan)}}</td>
                <td style="text-align: center">{{$item->norek}}</td>
